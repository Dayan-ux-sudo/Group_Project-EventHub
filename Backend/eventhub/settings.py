...
ALLOWED_HOSTS = [
    host.strip()
    for host in config("ALLOWED_HOSTS", default="localhost,127.0.0.1,[::1],backend").split(",")
    if host.strip()
]
...
DEFAULT_FROM_EMAIL = config("DEFAULT_FROM_EMAIL", default="no-reply@eventhub.local")
